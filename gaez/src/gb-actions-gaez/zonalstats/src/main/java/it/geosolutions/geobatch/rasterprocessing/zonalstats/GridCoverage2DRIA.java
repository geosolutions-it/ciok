 ...
 */
final class GridCoverage2DRIA extends GeometricOpImage {
    ...
    private MathTransform src2dstTransform;

    private MathTransform dst2srcTransform;
    ...
    protected GridCoverage2DRIA(final GridCoverage2D src, final GridCoverage2D dst,
            final Vector<RenderedImage> sources, final ImageLayout layout, @SuppressWarnings("rawtypes") final Map configuration,
        ...
        try {
            src2dstTransform=ConcatenatedTransform.create(ConcatenatedTransform.create(g2ws, src2dstCRSTransform), w2gd);
            dst2srcTransform=src2dstTransform.inverse();
        }  catch (NoninvertibleTransformException e) {
            throw new IllegalArgumentException("Can't create a transform between CRS", e);
        }
    }
    ...
    protected Rectangle forwardMapRect(Rectangle pxRect, int i) {
        ...
        Rectangle srcRect = pointArr2Rect(pts, interp);
    ...
    private void mapDestPoint(double[] coords) throws TransformException {
        ...
        if(src2dstTransform.isIdentity())
            return ;
//        src2dstTransform.transform(coords, 0, coords, 0, npoints);
    ...
    private void mapSrcPoint(double[] coords) throws TransformException {
        final int npoints = coords.length / 2;
        if(dst2srcTransform.isIdentity())
            return ;
        dst2srcTransform.transform(coords, 0, coords, 0, npoints);
...
//        if(!dst2srcCRSTransform.isIdentity())
...
//        // System.out.println(sb);
    ...
    protected Rectangle backwardMapRect(Rectangle destRect, int sourceIndex) {
        ...
        Rectangle pxRect = pointArr2Rect(pts, interp);
        return pxRect;
    }
    ...
    private static Rectangle pointArr2Rect(float[] points, Interpolation interp) {
