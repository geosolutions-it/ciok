...
public class RasterZonalStatistics {
    ...
    static class RasterZonalStatisticsIterator implements SimpleFeatureIterator {
            ...
            if(features.size() == 0) {
                ...
                try {
                    ...
                    if (!CRS.equalsIgnoreMetadata(zonesCrs, dataCrs)) {
                        final MathTransform transform=CRS.findMathTransform(zonesCrs, dataCrs,true);
                        if(!transform.isIdentity()){
                            zoneGeom = JTS.transform(zoneGeom, transform);
                        }
                    ...
                    throw new RuntimeException("Failed to compute statistics on feature " + zone, e);
                }
            } 
         ...
         * @param stats
         */
        private void addStatsToFeature(ZonalStats stats) {
...
//                    return null;
                ...
                Statistic[] requiredStatistics = new Statistic[] { Statistic.MAX, Statistic.MIN,
                        Statistic.RANGE, Statistic.MEAN, Statistic.SDEV, Statistic.SUM };
                final ParameterBlockJAI pbj = new ParameterBlockJAI("ZonalStats");
                pbj.setSource("dataImage", cropped.getRenderedImage());
                if(classificationRaster!=null)
                    pbj.setSource("zoneImage", classificationRaster);
                pbj.setParameter("stats", requiredStatistics);
                pbj.setParameter("bands", new Integer[] { band });
                pbj.setParameter("roi", roi);
                pbj.setParameter("noDataRanges", novalueRangeList);
                final RenderedOp zsOp = JAI.create("ZonalStats", pbj);
